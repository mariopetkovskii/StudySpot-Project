    ...
    public void confirmAccount(@RequestParam String token, HttpServletResponse response) throws IOException {
        ...
        if (tokenInDB != null) {
            if (tokenInDB.getExpirationDate().isBefore(OffsetDateTime.now())) {
                ...
                response.sendRedirect("http://51.141.9.231/login?message=tokenexpired");
            } else {
                User user = tokenInDB.getUser();
                if (user.getIsEnabled()) {
                    response.sendRedirect("http://51.141.9.231/login?message=exists");
                } else {
                    this.userService.enableAccount(user);
                    response.sendRedirect("http://51.141.9.231/login?message=success");
                }
            }
        } else {
            response.sendRedirect("http://51.141.9.231/login?message=invalidtoken");
        }
    }
